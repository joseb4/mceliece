...
def irreducible_poly(m, p, var):
    """Generate a random irreducible polynomial of degree ``m`` in ``GF(p)[x]``."""
...
def first_alpha_power_root(poly, irr_poly, p, elements_to_check=None):
    poly = Poly([(Poly(coeff, alpha, domain =GF(p)) % irr_poly).as_expr() for coeff in poly.all_coeffs()], x)
    # poly = Poly([(Poly(coeff, alpha, domain =GF(p)) % irr_poly).trunc(p).as_expr() for coeff in poly.all_coeffs()], x) #Old code line
    test_poly = Poly(1, alpha, domain = GF(p))
    log.debug(f"testing f:{poly}")
    for i in range(1, p ** irr_poly.degree()):
        test_poly = (Poly(Poly(alpha, alpha, domain = GF(p)) * test_poly, alpha) % irr_poly).set_domain(GF(p))
        if elements_to_check is not None and i not in elements_to_check:
            continue
        
        value = Poly((Poly(poly.eval(test_poly.as_expr()), alpha, domain = GF(p)) % irr_poly), alpha)
        # value = Poly((Poly(poly.eval(test_poly.as_expr()), alpha) % irr_poly), alpha).trunc(p) #Old code line
...
def is_alpha_power_root(poly, i, irr_poly, p):
    return Poly((Poly(poly.eval(alpha ** i), alpha, domain = GF(p)) % irr_poly), alpha).is_zero#.trunc(p).is_zero
    #return Poly((Poly(poly.eval(alpha ** i), alpha, domain = GF(p)) % irr_poly), alpha).trunc(p).is_zero
...
def minimal_poly(i, n, q, irr_poly):
    ...
    for k in range(n):
        ti = (ti * q) % n
        if checked[ti]:
            polys = [(Poly(c, alpha, domain = GF(2)) % irr_poly) for c in poly.all_coeffs()]
...
def power_dict(n, irr, p):
    # print("irr_pol: {}\n".format(irr))
    result = {(1,): 0} # 1 is always the first element
    test_poly = Poly(1, alpha, domain = GF(p)) #This is in ZZ. Alpha is a root of the polynomial
    # print("Adding 0, 1, alpha, alpha²= alpha *alpha, ..., alpha^(q^m-1) = alpha^(q^m-2)^alpha (mod irr_poly) to the ring\n")
    # print("test_poly: {}\n".format(test_poly))
    for i in range(1, n - 1):
        test_poly = (Poly(Poly(alpha, alpha, domain = GF(p)) * test_poly, alpha) % irr).set_domain(GF(p))
        print("alpha^{}: {}\n".format(i,test_poly)) 
...
def get_alpha_power(poly, irr_poly, quotient, p, neg=False): #Quotien field= 

    try:
        _, exp = poly.as_base_exp()
        print("exp", exp)
        poly = (Poly(alpha**exp, alpha).set_domain(GF(p)) % irr_poly)#.trunc(p)
        if poly.is_zero:
            return 0
        print(tuple(poly.all_coeffs()))
        ...
        poly = (Poly(poly, alpha, GF(p)) % irr_poly)#.trunc(p)
...
def get_binary_from_alpha(poly, irr_poly, p):
    poly = (Poly(poly, alpha, domain = GF(p)) % irr_poly)#.trunc(p)
...
def reduce_to_alpha_power(poly, irr_poly, quotient, p):
    print(quotient)
    # Split the polynomial into its coefficients, and reduce as powers of $\alpha$.
    reduced_coefficients = [get_alpha_power(coeff, irr_poly, quotient, p) for coeff in poly.all_coeffs()]
    print("Reduced coeffs: ", reduced_coefficients)
    return Poly(reduced_coefficients, x) # Coeff is a pow (base ** exponente)
    #return Poly(get_alpha_power(poly, irr_poly, quotient, p) , x) # Quotient = ring (quotient field)
