...
class GoppaCodeGenerator:
    ...
    def gen(self):
        ...
        if is_irreducible_poly(irr_poly, self.q):
            print("alpha^m +alpha +1  is irreducible ...")
            ring = power_dict(self.q ** self.m, irr_poly, self.q)
        else:
            print("Find another irreducible polynomial and create ring")
            ring = []
        
        while len(ring) < self.q ** self.m - 1:
            print(len(ring))
            print("Generate random irreducible polynomial of degree ``m`` in ``GF(p)[x]``")
            irr_poly = irreducible_poly(self.m, self.q, alpha)
            print("Calculating the ring ...")
            ring = power_dict(self.q ** self.m, irr_poly, self.q)
            log.info("irr(q_size: {}): {}".format(len(ring), irr_poly))
        print("Ring created ...\n")
        log.debug(f"ring={ring}")
        g_poly = Poly(1, x)
        print("Finding roots of irr_pol to generate g(x) = \sum c_i/(x-alpha_i) ...\n")
        ...
        print("g_roots", g_roots)
        print("g_non_roots", g_non_roots)
        ...
        log.debug(f"X={X}")
        print(" Generating matrix Y...")
        Y = Matrix(self.n, self.n, lambda i, j: get_alpha_power(g_poly.eval(alpha ** g_non_roots[i]), irr_poly, ring, self.q, neg=True)
                   if i == j else 0)
        log.debug(f"Y={Y}")
        H = C * X * Y
